...
public class RelativeFile extends RandomAccessFile {
    ...
    public void modifyField(int employeeNumber, int offset, String modifyValue) throws IOException {
        int relativeKey = this.calculateRelativeKey(employeeNumber);
        String validRecord = this.checkRecordValid(employeeNumber, relativeKey);
        switch(validRecord) {
            case "Yes":
                this.seek((relativeKey - 1) * recordLength + offset);
                this.writeBytes(modifyValue);
                break;
            case "No":
                System.out.println("Error Not A Valid Record");
                break;
    ...
    public void modifyField(int employeeNumber, int offset, float modifyValue) throws IOException {
        int relativeKey = this.calculateRelativeKey(employeeNumber);
        String validRecord = this.checkRecordValid(employeeNumber, relativeKey);
        switch(validRecord) {
            case "Yes":
                this.seek((relativeKey - 1) * recordLength + offset);
                this.writeFloat(modifyValue);
                break;
            case "No":
                System.out.println("Error Not A Valid Record");
                break;
